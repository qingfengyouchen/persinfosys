			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="retire/listRetire.do" method="post" name="userForm" id="userForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="width:160px;line-height:17.57px;padding-bottom: 1px;">退休时间</th>
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										<tr>
                                            ...
                                            <td class="center" style="width:160px;line-height:17.57px;padding-bottom: 1px;">${user.RETIRE_TIME }</td>
											...
											<td class="center" style="width:80px;line-height:17.57px;padding-bottom: 1px;">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-warning" title='发送短信' onclick="sendSms('${user.PHONE }');">
													 ...
													 -->
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="editUser('${user.ID_NUM}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="delUser('${user.ID_NUM }');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="editUser('${user.ID_NUM}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="delUser('${user.ID_NUM }');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增职员";
			 diag.URL = '<%=basePath%>retire/goAddU.do';
			 diag.Width = 500;
			 diag.Height = 619;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="职员资料";
			 diag.URL = '<%=basePath%>retire/goEditU.do?ID_NUM='+user_id;
			 diag.Width = 500;
			 diag.Height = 619;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId){
			bootbox.confirm("确定要删除["+userId+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>retire/deleteU.do?ID_NUM="+userId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>retire/deleteAllU.do?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		function toExcel(){
			...
			window.location.href="<%=basePath%>retire/excel.do?keywords=" + encodeURI(encodeURI(keywordsVal)) + "&userunit=" + encodeURI(encodeURI(userunitVal));
		}
		</script>
    </body>
</html>
