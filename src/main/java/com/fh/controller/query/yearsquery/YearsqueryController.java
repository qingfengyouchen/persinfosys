package com.fh.controller.query.yearsquery;
 ...
 */
@Controller
@RequestMapping(value="/yearsquery")
public class YearsqueryController extends BaseController {
	
	String menuUrl = "yearsquery/listYearsquery.do"; //菜单地址(权限用)
	@Resource(name="yearsqueryService")
	private YearsqueryManager yearsqueryService;
	
	/**显示用户列表
	 * @param page
	 * @return
	 */
	@RequestMapping(value="/listYearsquery")
	public ModelAndView listUsers(Page page){
		...
		try{
			...
			String usersyears = pd.getString("usersyears");	//检索条件 单位
//			if(null != userage && !"".equals(userage)){
//				pd.put("USERAGE", userage.trim());
//			}
			page.setPd(pd);
			List<PageData>	yearsqueryList1 = yearsqueryService.listPdPageUser(page);//列出会员列表
			List<PageData>	yearsqueryList = new ArrayList<PageData>(); //列出会员列表
			for (PageData tmp : yearsqueryList1) {
				//工龄对比
				String years = getSeniority(tmp.getString("WORK_DATE"));
				if(years.equals(usersyears)){
					// 年龄
			...
			mv.setViewName("query/yearsquery/yearsquery_list");
			if(null != usersyears && !"".equals(usersyears)){
				mv.addObject("yearsqueryList", yearsqueryList);
			}else{
				mv.addObject("yearsqueryList", yearsqueryList1);
			}
			mv.addObject("pd", pd);
	...
	public ModelAndView goDetailU(){
		...
		pd = this.getPageData();
		try {
			pd = yearsqueryService.findByUiId(pd); //根据ID读取
			// 工龄
	...
	public ModelAndView exportExcel(){
		...
		try{
			if(Jurisdiction.buttonJurisdiction(menuUrl, "cha")){	
				...
				List<PageData> userList = yearsqueryService.listAllUser(pd);
